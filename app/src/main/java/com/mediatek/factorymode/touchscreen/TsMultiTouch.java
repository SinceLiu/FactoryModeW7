...
public class TsMultiTouch extends BaseTestActivity {
            ...
            new Thread() {
                public void run() {
                    ...
                    try {
                        ...
                        if (0 == ret) {
                            Log.v(TAG, "-->onResume Start logging...");

                        } else {
                            Log.v(TAG, "-->onResume Logging failed!");
                        }
                    } catch (IOException e) {
    ...
    public class MyView extends View {
        ...
        protected void onDraw(Canvas canvas) {
            ...
            for (int idx = 0; idx < fingerNum; idx++) {
                ...
                Log.i(TAG, "idx: " + idx + " input size: " + inputSize);
                for (int j = 0; j < inputSize; j++) {
                    Vector<TsPointDataStruct> line = inputIdx.get(j);
                    int lineSize = line.size();
                    Log.i(TAG, "Line" + j + " size " + lineSize);
                    if (lineSize > 0) {
...
//                                    + ", y=" + String.valueOf(last.coordinateY);
        ...
        public boolean onTouchEvent(MotionEvent event) {
            ...
            Log.i(TAG, "onTouchEvent: ptIdx: " + ptIdx + " mPtsStatus.size(): " + mPtsStatus.size());
...
//                    pt.isDown = false;
            ...
            for (int idx = 0; idx < mPtsStatus.size(); idx++) {
                ...
                Log.i(TAG, "mPtsStatus.size(): " + mPtsStatus.size()
...
//                    st.setmNewLine(false);
            ...
            Log.i(TAG, "Pointer counts = " + pointCt);
            for (int i = 0; i < pointCt; i++) {
                int notZeroBasedPid = event.getPointerId(i);
                calcMinId(notZeroBasedPid);
                Log.i(TAG, " i =" + i + " notZeroBasedPid = " + notZeroBasedPid + " mMinPtrId = " + mMinPtrId);
...
//                    float tmpSize = event.getSize(pid);
                    ...
                    Log.i(TAG, " pid = " +  pid + " mInputIds.size() = " + mInputIds.size());
//                    Xlog.i(TAG, " mInputIds.get(pid).size() = " +  mInputIds.get(pid).size());
                    if (i < mInputIds.size()) {
                        Log.i(TAG, " mInputIds.get(i).size() = "
