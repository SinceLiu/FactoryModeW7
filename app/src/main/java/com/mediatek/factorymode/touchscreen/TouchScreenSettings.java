...
public class TouchScreenSettings extends BaseTestActivity implements OnClickListener {
...
//        Xlog.v(TAG, "-->onCreate");
    ...
    private String getFileValue(String path) {
        ...
        try {
            ...
            Log.v(TAG, "-->GetFileValue:" + e.getMessage());
            return "N/A";
        }
    }
...
//    }
    ...
    class WorkThread extends Thread {
        ...
        public void run() {
            while (sRun) {
                ...
                if (mSdcardExist) {
                    ...
                    String shell = "cat /sys/module/tpd_debug/parameters/tpd_em_log  >> "
                            + sCurrentFileName;
                    Log.v(TAG, "run file shell--" + shell);
                    String[] cmd2 = { "/system/bin/sh", "-c", shell };
                    int ret = 0;
                    try {
                        ret = TouchScreenShellExe.execCommand(cmd2);
                        if (0 != ret) {
                            Log.i(TAG, "cat >> failed!! ");
                            // return;
                        }
                    } catch (IOException e) {
                        Log.w(TAG, "cat >> failed!!  io exception");
                    }
                }
            ...
            Log.i(TAG, "Copy /proc/tpd_em_log success");
        }
    }
...
//                        if (editString.equals("0")) {
                        // close file log
                        sRun = false;
                        Log.i(TAG, "close file log mRun = " + sRun);
                        setCategory("0");
...
//                            setLastError(ERR_ERR);
                        ...
                        if (!touchLog.isDirectory()) {
                            touchLog.mkdirs();
                            Log.i(TAG, "mkdir " + touchLog.getPath()
                                    + " success");
                        }
                        ...
                        Log.v(TAG, "thread start mRun = " + sRun);
                        Toast.makeText(this, "Start log file to sdcard.",
                                Toast.LENGTH_SHORT).show();
                        Log.v(TAG, "Start log file to sdcard.");
                        SharedPreferences.Editor editor = preferences.edit();
...
//                        if (editString.equals("0")) {
                        sRun = false;
                        Log.i(TAG, "uart close mRun = " + sRun);
                        final SharedPreferences preferences = this
                                .getSharedPreferences("touch_screen_settings",
                                        android.content.Context.MODE_PRIVATE);
                        SharedPreferences.Editor editor = preferences.edit();
...
//            setLastError(ERR_ERR);
    ...
    public void runFirstCommand() {
        try {
            ...
            Log.v(TAG, "write tpd_mode result:"
